...
export default defineConfig(({ mode }) => {
    ...
    const clientHost = envVars.VITE_CLIENT_HOST ||
        envVars.VITE_HOST ||
        envVars.CLIENT_HOST ||
        envVars.HOST ||
        "0.0.0.0";
    const clientPort = Number(envVars.VITE_CLIENT_PORT ||
        envVars.VITE_PORT ||
        envVars.CLIENT_PORT ||
        envVars.PORT_CLIENT ||
        3000);
    const hmrPort = Number(envVars.VITE_HMR_PORT ||
        envVars.HMR_PORT ||
        envVars.WS_PORT ||
        clientPort + 1);
    // Resolve API backend target
    const backendPort = Number(envVars.BACKEND_PORT ||
        envVars.SERVER_PORT ||
        envVars.API_PORT ||
        envVars.PORT ||
        4005);
    const explicitApiUrl = envVars.VITE_PROXY_TARGET ||
