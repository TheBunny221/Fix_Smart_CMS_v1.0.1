...
class ComplaintEmailBroadcaster {
  ...
  async generateEmailContent({
    ...
    const subject = this.generateSubjectLine(complaint, newStatus, recipient.role, recipient.language);
   ...
   */
  generateSubjectLine(complaint, status, role, language = 'en') {
    const complaintId = complaint.complaintId || `#${complaint.id.slice(-6)}`;
    const appName = systemConfigCache.get('APP_NAME', 'Fix_Smart_CMS');
