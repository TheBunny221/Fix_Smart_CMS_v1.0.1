    ...
    try {
        ...
        if (format === "csv") {
            ...
            const csvRows = complaints.map((complaint) => {
                // Format complaint ID with system prefix
                const complaintId = `${complaintIdPrefix}-${String(complaint.id).padStart(6, "0")}`;

                // Determine SLA status
                let slaStatus = "Active";
                if (complaint.status === "CLOSED" && complaint.closedOn && complaint.submittedOn) {
                    // Calculate if resolved within SLA
                    const slaHours = complaint.complaintType?.slaHours || 48;
                    const targetTime = new Date(complaint.submittedOn.getTime() + slaHours * 60 * 60 * 1000);
                    slaStatus = complaint.closedOn <= targetTime ? "Met" : "Breached";
                } else if (complaint.deadline && new Date() > new Date(complaint.deadline)) {
                    slaStatus = "Overdue";
                }
                ...
                return [
                    complaintId,
                    complaint.id,
                    complaint.complaintType?.name || complaint.type || "N/A",
            ...
            // Generate enhanced filename with filter context
        ...
        if (format === "pdf") {
            try {
                ...
                // Add header
                doc.fontSize(20).text(appName, { align: 'center' });
                ...
                // Add complaints data (simplified for PDF)
                complaints.slice(0, 50).forEach((complaint, index) => {
                    if (doc.y > 700) {
                        doc.addPage();
                    }

                    const complaintId = `${complaintIdPrefix}-${String(complaint.id).padStart(6, "0")}`;
                    doc.text(`${index + 1}. ${complaintId} - ${complaint.complaintType?.name || complaint.type}`);
                    doc.text(`   Status: ${complaint.status} | Priority: ${complaint.priority || 'N/A'}`);
                    doc.text(`   Ward: ${complaint.ward?.name || 'N/A'} | Submitted: ${complaint.submittedOn?.toLocaleDateString() || 'N/A'}`);
                    doc.moveDown(0.5);
                });

                if (complaints.length > 50) {
                    doc.text(`... and ${complaints.length - 50} more complaints`);
                }
            ...
            try {
                ...
                const excelData = complaints.map((complaint) => {
                    ...
                    return {
                        ...
                        'Type': complaint.complaintType?.name || complaint.type || "N/A",
                        'Description': complaint.description?.substring(0, 100) || "N/A",
                        'Status': complaint.status,
                        'Priority': complaint.priority || "N/A",
                        'Ward': complaint.ward?.name || "N/A",
                        'Submitted On': complaint.submittedOn?.toISOString().split("T")[0] || "N/A",
                        'Resolved On': complaint.resolvedOn ? complaint.resolvedOn.toISOString().split("T")[0] : "N/A",
