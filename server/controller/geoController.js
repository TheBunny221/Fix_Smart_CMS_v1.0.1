...
const buildHeaders = async (req) => {
  // Use SystemConfig instead of process.env for CONTACT_EMAIL
  const { getSystemConfig } = await import('./systemConfigController.js');
  const contactEmail = await getSystemConfig('CONTACT_EMAIL') || "support@cochinsmartcity.in";
  const ua = `NLC-CMS/1.0 (${contactEmail})`;
  return {
    "User-Agent": ua,
