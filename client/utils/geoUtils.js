 ...
 */
export function isPointInBoundingBox(point, boundingBox) {
    return (point.lat >= boundingBox.south &&
 ...
 */
export function detectLocationArea(coordinates, wards) {
    ...
    if (matchedWard && matchedWard.subZones) {
        for (const subZone of matchedWard.subZones) {
            if (!subZone.boundaries)
                continue;
            try {
                const boundaries = JSON.parse(subZone.boundaries);
                // Quick bounding box check first (if available)
 ...
 */
export function findNearestArea(coordinates, wards) {
    let nearestWard = null;
    let nearestSubZone = null;
    let minDistance = Infinity;
    for (const ward of wards) {
        if (ward.centerLat && ward.centerLng) {
            const distance = calculateDistance(coordinates, { lat: ward.centerLat, lng: ward.centerLng });
                ...
                if (ward.subZones) {
                    ...
                    for (const subZone of ward.subZones) {
                        if (subZone.centerLat && subZone.centerLng) {
                            const subZoneDistance = calculateDistance(coordinates, { lat: subZone.centerLat, lng: subZone.centerLng });
 ...
 */
export function simplifyPolygon(coordinates, tolerance = 0.0001) {
