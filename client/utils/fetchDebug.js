...
export function debugFetchEnvironment() {
    ...
    if (window.fetch) {
        ...
        else if (fetchString.includes("fullstory") ||
...
export function logFetchDebugInfo() {
    ...
    if (info.thirdPartyOverrides.length > 0) {
        console.warn("⚠️ Third-party libraries may be interfering with fetch requests");
    }
    console.groupEnd();
    return info;
}
...
export function createRobustFetch() {
    return async function robustFetch(input, init) {
        ...
        try {
            ...
            console.warn("Current fetch failed, trying XMLHttpRequest fallback:", error);
