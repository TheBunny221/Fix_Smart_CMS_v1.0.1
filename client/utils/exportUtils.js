...
const formatComplaintId = (dbId, prefix = "KSC") => {
    if (!dbId)
...
export const exportToPDF = async (data, trendsData, categoriesData, options) => {
    try {
        ...
        const addFooter = (pageNum, totalPages) => {
            ...
            doc.setFontSize(8);
            doc.text(`Generated by ${options.systemConfig.appName} | Page ${pageNum} of ${totalPages}`, pageWidth / 2, pageHeight - 10, { align: "center" });
        };
        // Start first page
        yPosition = addHeader();
        // Title section
        ...
        doc.setFontSize(14);
        doc.setFont("helvetica", "normal");
        doc.text(`Comprehensive Analytics & Data Export`, pageWidth / 2, yPosition, {
            align: "center",
        });
        yPosition += 25;
        // Report metadata box
        ...
        const actualResolved = data.complaints.filter((c) => String(c.status).toUpperCase() === "RESOLVED").length;
        const actualPending = data.complaints.filter((c) => ["REGISTERED", "ASSIGNED", "IN_PROGRESS"].includes(String(c.status).toUpperCase())).length;
...
export const exportToExcel = async (data, trendsData, categoriesData, options) => {
    try {
        ...
        const complaintsData = data.complaints.map((complaint) => {
            ...
            return {
                ...
                "Resolution Days": complaint.resolvedOn && complaint.createdAt
                    ? Math.ceil((new Date(complaint.resolvedOn).getTime() -
        ...
        if (trendsData.length > 0) {
            const trendsSheet = XLSX.utils.json_to_sheet(trendsData.map((trend) => ({
                ...
                "Resolution Rate %": trend.resolved > 0
...
export const validateExportPermissions = (userRole, requestedData) => {
    switch (userRole) {
        case "ADMINISTRATOR":
...
export const formatDataForChart = (data, dateField = "createdAt") => {
...
export const generateFileName = (appName, filters, format) => {
