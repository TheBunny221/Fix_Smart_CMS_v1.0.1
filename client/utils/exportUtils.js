...
const formatComplaintId = (
  ...
  if (typeof dbId === "string") {
    ...
    numericId = match ? parseInt(match[0]) ;
  } else {
    numericId = dbId;
  }
...
export const exportToPDF = async (
  ...
  const addFooter = (pageNum, totalPages) => {
    ...
    doc.setFontSize(8);
    doc.text(`Generated by ${options.systemConfig.appName} | Page ${pageNum} of ${totalPages}`,
      pageWidth / 2,
      pageHeight - 10,
      { align,
    );
  ...
  if (categoriesData.length > 0) {
    ...
    categoriesData.forEach((category) => {
      ...
      doc.text(`• ${categoryName}) - Avg: ${category.avgTime.toFixed(1)} days`,
        marginLeft + 5,
        yPosition,
      );
      yPosition += 6;
  ...
  summarySheet["merges"].push({ s, c: 0 }, e: { r, c: 3 } }); // Merge title
...
// Utility function to capture chart for PDF inclusion
...
export const formatDataForChart = (
  ...
  data.forEach((item) => {
    ...
    if (groupedData.has(date)) {
      groupedData.set(date, { date, count, resolved);
    }
