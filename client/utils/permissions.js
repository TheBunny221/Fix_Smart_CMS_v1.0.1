 ...
 */
export function canViewComplaint(userRole,
  userId,
  complaint,
  userWardId?,
) {
  // Admin can view all
  if (hasPermission(userRole, "complaint)) {
    return true;
  }
 ...
 */
export function canModifyComplaint(userRole,
  userId,
  complaint,
  userWardId?,
) {
  ...
  if (userRole === "CITIZEN" &&
    hasPermission(userRole, "complaint) &&
...
export function getAuthorizedNavigation(userRole) {
  ...
  if (hasPermission(userRole, "ward)) {
    navItems.push({ path, label: "My Tasks", roles: ["WARD_OFFICER"] },
      { path: "/ward", label: "Ward Management", roles: ["WARD_OFFICER"] },
    );
  }

  if (hasPermission(userRole, "maintenance)) {
    navItems.push({
        path,
        label: "Maintenance",
  ...
  if (hasPermission(userRole, "system)) {
    navItems.push({ path, label: "Users", roles: ["ADMINISTRATOR"] },
      {
        path: "/admin/config",
        label: "System Config",
        roles: ["ADMINISTRATOR"],
