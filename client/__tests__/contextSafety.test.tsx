...
import {
  useSafeContext,
  useSafeOptionalContext,
} from "../hooks/useSafeContext";
...
const TestSystemConfigComponent = () => {
  ...
  return (
    <div>
      ...
      <span data-testid="custom-config">
        {getConfig("CUSTOM_KEY", "default")}
      </span>
    </div>
  );
};
...
describe("Context Safety Tests", () => {
  ...
  describe("ContextErrorBoundary", () => {
    ...
    it("should catch context-related errors", () => {
      ...
      expect(
        screen.getByText("TestContext context is not available."),
      ).toBeInTheDocument();
  ...
  describe("Safe Context Hooks", () => {
    const TestContext = React.createContext<{ value: string } | undefined>(
      undefined,
    );

    const TestSafeContextComponent = () => {
      const safeValue = useSafeOptionalContext(TestContext, "TestContext", {
        value: "fallback",
      });
