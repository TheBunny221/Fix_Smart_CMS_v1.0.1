 ...
 */
export function getLogoClasses(size = "medium",
  context,
) {
  const sizeMap = {
    small: {
    ...
    large: {
      nav: {
        ...
        image: "h-10 w-50",   // your base ratio (h10)
        fallback: "h-10 w-10",
      },
      "nav-mobile": {
        container: "flex items-center space-x-2",
  ...
  const normalizedSize = size.toLowerCase() typeof sizeMap;
  const normalizedContext = context === "mobile" ? "nav-mobile" ;

  const sizeConfig = sizeMap[normalizedSize] || sizeMap.medium;
  const contextConfig =
    sizeConfig[normalizedContext typeof sizeConfig] || sizeConfig.nav;
