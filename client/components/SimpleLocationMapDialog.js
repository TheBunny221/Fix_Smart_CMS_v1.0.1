...
const injectLeafletCssOnce = (() => {
    ...
    return () => {
        ...
        if (typeof document === "undefined")
            return;
        const href = "https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.css";
        // Avoid duplicates
        if (Array.from(document.head.querySelectorAll("link")).some((l) => l.href === href)) {
            return;
        }
...
const SimpleLocationMapDialog = ({ isOpen, onClose, onLocationSelect, initialLocation, }) => {
    const { getConfig, isLoading: configLoading } = useSystemConfig();
    ...
    const defaultLat = parseFloat(getConfig("MAP_DEFAULT_LAT", KOCHI_DEFAULT_LAT.toString())) ||
        KOCHI_DEFAULT_LAT;
    const defaultLng = parseFloat(getConfig("MAP_DEFAULT_LNG", KOCHI_DEFAULT_LNG.toString())) ||
        KOCHI_DEFAULT_LNG;
    const mapPlace = getConfig("MAP_SEARCH_PLACE", "Kochi, Kerala, India");
    const countryCodes = getConfig("MAP_COUNTRY_CODES", "in").trim();
    const bboxNorth = parseFloat(getConfig("MAP_BBOX_NORTH", KOCHI_BBOX.north.toString())) ||
        KOCHI_BBOX.north;
    const bboxSouth = parseFloat(getConfig("MAP_BBOX_SOUTH", KOCHI_BBOX.south.toString())) ||
        KOCHI_BBOX.south;
    const bboxEast = parseFloat(getConfig("MAP_BBOX_EAST", KOCHI_BBOX.east.toString())) ||
        KOCHI_BBOX.east;
    const bboxWest = parseFloat(getConfig("MAP_BBOX_WEST", KOCHI_BBOX.west.toString())) ||
        KOCHI_BBOX.west;
    ...
    const retryCenterRef = useRef(null);
    const [mapInitialized, setMapInitialized] = useState(false);
    const [mapReloadKey, setMapReloadKey] = useState(0);
    const [address, setAddress] = useState(initialLocation?.address || "");
    const [area, setArea] = useState(initialLocation?.area || "");
    ...
    const [detectedSubZone, setDetectedSubZone] = useState("");
    const [isDetectingArea, setIsDetectingArea] = useState(false);
    ...
    const isWithinKochiBoundary = useCallback((lat, lng) => {
        return lat >= bboxSouth && lat <= bboxNorth && lng >= bboxWest && lng <= bboxEast;
    }, [bboxNorth, bboxSouth, bboxWest, bboxEast]);
    const showBoundaryError = useCallback((action) => {
        const message = `${action} is outside Kochi service area. Please select a location within Kochi.`;
        setMapError(message);
        toast({
            title: "Location Outside Service Area",
    ...
    const setPositionWithValidation = useCallback((newPos, action = "Selected location") => {
        if (!isWithinKochiBoundary(newPos.lat, newPos.lng)) {
            showBoundaryError(action);
            return false;
        }
    ...
    useEffect(() => {
        ...
        const initializeMap = async () => {
            try {
                ...
                leafletMapRef.current._resizeObserver = resizeObserver;
            }
            catch (err) {
                if (!cancelled) {
                    console.error("Error initializing map:", err);
                    setMapError(`Failed to load map: ${err instanceof Error ? err.message : "Unknown error"}. Please retry.`);
                    setMapInitialized(false);
                }
            }
        };
        ...
        return () => {
            ...
            try {
                ...
                const resizeObserver = leafletMapRef.current?._resizeObserver;
                if (resizeObserver) {
        ...
        try {
            ...
            if (data?.display_name) {
                ...
                if (seq === detectionSeqRef.current && areaSource !== "api") {
                    ...
                    const detectedArea = a.neighbourhood ||
                        a.suburb ||
                        a.city_district ||
                        a.state_district ||
                        a.city ||
    ...
    const searchLocation = useCallback((immediate = false) => {
        ...
        const doSearch = async () => {
            ...
            const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&limit=5&addressdetails=1${viewbox}${cc}`;
            try {
                const res = await fetch(url, {
                    signal,
                    headers: {
    ...
    return (_jsx(Dialog, { open: isOpen, onOpenChange: onClose, children: _jsxs(DialogContent, { className: "max-w-4xl w-[95vw] max-h-[95vh] flex flex-col p-0", children: [_jsx(DialogHeader, { className: "px-6 py-4 border-b shrink-0", children: _jsxs(DialogTitle, { className: "flex items-center gap-2", children: [_jsx(MapPin, { className: "h-5 w-5" }), "Select Location on Map"] }) }), _jsx("div", { className: "flex-1 overflow-y-auto px-6 py-4", children: _jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "flex flex-col sm:flex-row gap-2", children: [_jsxs("div", { className: "flex-1 flex gap-2", children: [_jsx(Input, { placeholder: `Search for a location in ${mapPlace}`, value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), onKeyDown: handleSearchKey, className: "flex-1" }), _jsx(Button, { onClick: () => searchLocation(true), variant: "outline", size: "icon", className: "shrink-0", "aria-label": "Search location", children: _jsx(Search, { className: "h-4 w-4" }) })] }), _jsxs("div", { className: "flex gap-2", children: [_jsxs(Button, { onClick: getCurrentLocation, variant: "outline", disabled: isLoadingLocation, className: "flex items-center gap-2 whitespace-nowrap", children: [_jsx(Navigation, { className: "h-4 w-4" }), isLoadingLocation ? "Getting..." : "Current Location"] }), _jsx(Button, { onClick: handleDropPinHere, variant: "outline", className: "whitespace-nowrap", children: "Drop Pin Here" }), _jsx(Button, { onClick: () => {
                                                    handleRetryMap();
                                                }, variant: "outline", size: "sm", className: "whitespace-nowrap", children: "Retry Map" })] })] }), _jsx("div", { className: "h-64 sm:h-80 lg:h-96 w-full rounded-lg overflow-hidden border relative bg-gray-100", children: mapError ? (_jsx("div", { className: "h-full flex items-center justify-center bg-gray-50 relative z-[9999]", children: _jsxs("div", { className: "text-center p-4 max-w-md bg-white rounded-lg shadow-lg border border-red-200 z-[9999]", children: [_jsx(AlertCircle, { className: "h-8 w-8 text-red-500 mx-auto mb-2" }), _jsx("p", { className: "text-red-600 text-sm mb-3 font-medium", children: mapError }), _jsxs("div", { className: "flex flex-col sm:flex-row gap-2 justify-center", children: [_jsx(Button, { onClick: getCurrentLocation, variant: "outline", size: "sm", disabled: isLoadingLocation, className: "z-[9999] relative", children: isLoadingLocation ? "Getting Location..." : "Try Current Location" }), _jsx(Button, { onClick: handleRetryMap, variant: "outline", size: "sm", className: "z-[9999] relative", children: "Retry Map" })] })] }) })) : (_jsxs(_Fragment, { children: [(!mapInitialized || configLoading) && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-gray-100 z-[9999]", children: _jsxs("div", { className: "text-center bg-white rounded-lg shadow-lg p-4 border", children: [_jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-2" }), _jsx("p", { className: "text-sm text-gray-600", children: configLoading ? "Loading configuration..." : "Loading map..." })] }) })), _jsx("div", { ref: mapRef, className: "h-full w-full relative", style: {
                                                minHeight: "256px",
                                                background: "#f0f0f0",
                                                cursor: mapInitialized ? "crosshair" : "default",
                                            }, "aria-label": "Map container" }), mapInitialized && (_jsx("div", { className: "pointer-events-none absolute inset-0 flex items-center justify-center z-[1000]", children: _jsxs("div", { className: "relative", children: [_jsx("div", { className: "absolute inset-0 rounded-full border-2 border-blue-400 opacity-30 animate-ping", style: { width: '32px', height: '32px', top: '-2px', left: '-2px' } }), _jsxs("svg", { width: "32", height: "32", viewBox: "0 0 32 32", className: "text-gray-700 drop-shadow-lg", children: [_jsx("circle", { cx: "16", cy: "16", r: "6", fill: "white", fillOpacity: "0.9", stroke: "#e5e7eb", strokeWidth: "1" }), _jsx("circle", { cx: "16", cy: "16", r: "2", fill: "currentColor" }), _jsx("line", { x1: "16", y1: "2", x2: "16", y2: "8", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("line", { x1: "16", y1: "24", x2: "16", y2: "30", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("line", { x1: "2", y1: "16", x2: "8", y2: "16", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("line", { x1: "24", y1: "16", x2: "30", y2: "16", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("circle", { cx: "16", cy: "16", r: "5", stroke: "currentColor", strokeWidth: "1.5", fill: "none", opacity: "0.8" })] }), _jsx("div", { className: "absolute top-8 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded whitespace-nowrap opacity-75", children: "Click to place pin here" })] }) })), mapInitialized && (_jsx("div", { className: "absolute top-2 right-2 bg-white/95 backdrop-blur-sm rounded-md px-3 py-2 text-xs text-gray-700 shadow-lg border z-[1001]", children: "\uD83D\uDCCD Click map or drag marker to select location" })), mapInitialized && (_jsx("div", { className: "absolute bottom-2 left-2 bg-blue-50/95 backdrop-blur-sm rounded-md px-3 py-2 text-xs text-blue-700 shadow-lg border border-blue-200 z-[1001]", children: "\uD83D\uDDFA\uFE0F Service area: Kochi boundaries" }))] })) }), _jsxs("div", { className: "space-y-2 relative z-[1002]", children: [isLoadingLocation && (_jsxs("div", { className: "flex items-center gap-2 text-sm text-blue-600 bg-blue-50 p-3 rounded-lg border border-blue-200 shadow-sm relative z-[1002]", children: [_jsx("div", { className: "animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600" }), "Getting your current location..."] })), isDetectingArea && (_jsxs("div", { className: "flex items-center gap-2 text-sm text-orange-600 bg-orange-50 p-3 rounded-lg border border-orange-200 shadow-sm relative z-[1002]", children: [_jsx("div", { className: "animate-spin rounded-full h-4 w-4 border-b-2 border-orange-600" }), "Detecting administrative area..."] })), _jsx("div", { className: "bg-gray-50 p-3 rounded-lg border border-gray-200 relative z-[1002]", children: _jsxs("p", { className: "text-xs text-gray-600", children: ["\uD83D\uDCA1 ", _jsx("strong", { children: "Tips:" }), " Click anywhere on the map, drag the marker, use \"Drop Pin Here\" to place the pin at the map center, or use your current location."] }) })] }), _jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-4", children: [_jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "detected-area", children: "Detected Area" }), _jsx(Input, { id: "detected-area", value: area, onChange: (e) => setArea(e.target.value), placeholder: "Area/Locality" })] }), _jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "landmark", children: "Landmark (Optional)" }), _jsx(Input, { id: "landmark", value: landmark, onChange: (e) => setLandmark(e.target.value), placeholder: "Nearby landmark" })] }), _jsxs("div", { className: "space-y-2 sm:col-span-2", children: [_jsx(Label, { htmlFor: "detected-address", children: "Detected Address" }), _jsx(Input, { id: "detected-address", value: address, onChange: (e) => setAddress(e.target.value), placeholder: "Full address", className: "w-full" })] })] }), _jsxs("div", { className: "text-sm text-muted-foreground bg-muted/50 p-3 rounded-lg", children: [_jsx("strong", { children: "Selected coordinates:" }), " ", position.lat.toFixed(6), ", ", position.lng.toFixed(6)] })] }) }), _jsx(DialogFooter, { className: "px-6 py-4 border-t shrink-0 bg-background", children: _jsxs("div", { className: "flex flex-col sm:flex-row gap-2 w-full sm:w-auto sm:ml-auto", children: [_jsx(Button, { variant: "outline", onClick: onClose, className: "w-full sm:w-auto", children: "Cancel" }), _jsx(Button, { onClick: handleConfirm, className: "w-full sm:w-auto", children: "Confirm Location" })] }) })] }) }));
};
