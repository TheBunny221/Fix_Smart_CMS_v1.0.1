...
if (typeof window !== "undefined" &&
    window.fetch &&
    !globalThis.__originalFetch) {
    globalThis.__originalFetch = window.fetch;
}
...
const baseQueryWithReauth = async (args, api, extraOptions) => {
    ...
    try {
        ...
        const response = await robustFetch(`/api${url.startsWith("/") ? "" : "/"}${url}`, options);
...
export const optimisticUpdate = (items, updatedItem, idField = "id") => {
    return items.map((item) => item[idField] === updatedItem.id ? { ...item, ...updatedItem } : item);
};
// Helper for handling pessimistic updates (rollback on error)
export const rollbackUpdate = (items, originalItem, idField = "id") => {
    return items.map((item) => item[idField] === originalItem[idField] ? originalItem : item);
};
